...
const badCatchIndex = lines.findIndex((l, i) => i > 400 && l.includes('} catch (err) {'));
const schemaStart = lines.findIndex((l, i) => i > 400 && l.includes('const schemaData = [];'));

if (badCatchIndex !== -1 && schemaStart !== -1) {
    const top = lines.slice(0, badCatchIndex + 1);
    const bottom = lines.slice(schemaStart);
    
    let fillInStr = `                console.error("❌ Error subiendo media:", err);
    ...
    const newLines = [...top, ...fillInStr.split('\n'), ...bottom];
    fs.writeFileSync(path, newLines.join('\n'), 'utf8');
    console.log("Fixed server.js correctly.");
} else {
    console.log("Could not find boundaries.", badCatchIndex, schemaStart);
}
